...
loadDotEnvFile((0, node_path_1.resolve)(process.cwd(), '.env'));
const config = {
...
const lastSentByVehicle = new Map();
...
function extractEvent(message) {
    ...
    const externalVehicleId = asString(msg.vehicle_id) ?? asString(msg.vehicleId) ?? asString(vehicleObj?.id);
    ...
    const hasTelemetryValue = typeof lat === 'number' ||
        typeof lng === 'number' ||
...
function shouldThrottle(vehicleKey) {
    const now = Date.now();
...
async function postToParkSignal(event) {
